...
La variable a utilizar es la Tasa de Desempleo (sin desestacionalizar) de Colombia, calculada y publicada por el Departamento Administrativo Nacional de Estadísticas (DANE). La variable está en porcentaje, para un periodo desde enero de 2001 hasta diciembre de 2020 (240 observaciones) con una frecuencia mensual.

## 1.1. Importar Base

```{r importar}
library(readxl)
desempleo = read_excel("C:/Users/Rodro/Downloads/TD.xlsx", sheet = "Hoja1")
```

## 1.2. Definición de varibales

```{r def}
td    = ts(desempleo$TD, start = c(2001,1), end = c(2020,12), frequency = 12)
...
A continuación, se busca realizar un pronóstico con submuestra de prueba. Es decir, se re-estima el modelo hasta 2020-11 y la proyección se realiza para 2020-12.
...
Fecha2 = seq(as.Date("2021-01-01"), as.Date("2021-06-01"), by = "months") # secuencia
